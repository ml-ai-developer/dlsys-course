    ...
    def run(self, feed_dict):
        ...
        for node in topo_order:
            if node in node_to_val_map:
                continue
            in_vals = [node_to_val_map[n] for n in node.inputs]
    ...
    for node in reverse_topo_order:
        ...
        for i, in_node in enumerate(node.inputs):
            if in_node in node_to_output_grads_list:
                node_to_output_grads_list[in_node].append(grads[i])
            else:
                node_to_output_grads_list[in_node] = [grads[i]]
